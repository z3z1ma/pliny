...
When an executable ticket includes high-impact semantics or combines behavior from inspected records, source-observed facts, and current user ratification, include a compact assumption provenance section. Classify execution-relevant claims as record-backed, source-observed but not product-ratifying, user-ratified in the current workstream, or blocked. Do not add this section to trivial tickets with a single obvious provenance source. No unresolved assumption may appear in an executable ticket if it could change implementation or acceptance; if a blocked item remains, the ticket is a shaping or blocked ticket, not executable.
