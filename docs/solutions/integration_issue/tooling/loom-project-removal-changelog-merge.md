---
title: Removed LOOM_PROJECT.md; merged LOOM_CHANGELOG.md into LOOM_ROADMAP.md
...
# Removed LOOM_PROJECT.md; merged LOOM_CHANGELOG.md into LOOM_ROADMAP.md
...
- `LOOM_ROADMAP.md` described direction.
...
1) Choose a single source of truth for direction + history: `LOOM_ROADMAP.md`.
...
- the top of `LOOM_ROADMAP.md` (if it directly clarifies the roadmap).

3) Merge `LOOM_CHANGELOG.md` into `LOOM_ROADMAP.md` under a stable, greppable section header (for example: `## Changelog`).

4) Delete `LOOM_PROJECT.md` and stop referencing it.

5) Update internal pointers so every place that previously linked to either file now links to `LOOM_ROADMAP.md`:
...
- Add a lightweight contract check (test or CI) that asserts:
  ...
  - `LOOM_ROADMAP.md` contains `## Changelog`
  - `AGENTS.md` (and any templates) do not reference removed paths
- In new docs, link to `LOOM_ROADMAP.md` only (avoid introducing parallel "source of truth" files).
