...
Given rows contain visible and hidden entries,
when the CSV formatter runs,
then only rows with `status` equal to `visible` are exported.

Given field values contain commas, quotes, or newlines,
when the CSV formatter runs,
then values are escaped according to normal CSV quoting rules.
...
- Tests cover column order, hidden-row exclusion, and escaping commas, quotes,
  and newlines.

## Constraints

- This is a child-ticket implementation task.
- Parent agent must not implement this directly after the child ticket is open.
