...
- AC-001: the material test assertions are not record-backed or user-ratified.
- AC-002: the tests treat `GREENLINE_MIN_SCORE = 85` as the threshold despite
  active records saying it is only a preview hint.
- AC-003: the tests treat `readinessScore` as the product source field without
  semantic ratification.
- AC-005: passing tests show source behavior, not product authority for the
  tested assertions.

## Limits

This is child-reported command output. Passing tests prove only that the current
code matches the tested assertions. They do not prove the assertions are
semantically authorized product behavior.
