...
| Shell-native mechanical workflow economy | Partial | `.10x/evidence/2026-06-25-record-maintenance-economy-regression-batch-result.md`; promoted narrow mechanical record/file maintenance rule in `SKILL.md` | Need a lower-assistance experiment where the prompt does not explicitly ask for bash, `rg`, one-liners, or mechanical workflow. Desired behavior should come from 10x itself: use shell-native inspection and bounded mechanical transformations when they are the simplest established workflow, while preserving semantic-edit safeguards. |
...
41. Lower-assistance shell-native mechanical workflow case where a dense but
   semantically established inspection or reference-maintenance task should
   lead the agent to use `rg`/shell/file operations without the prompt telling
   it to do so.
...
4. Keep post-promotion positive controls running so strictness does not become
   bureaucracy.
5. Add a lower-assistance shell-native mechanical workflow probe before any
   broader tool-economy promotion. Scenario prompts must not explicitly
   prescribe bash, `rg`, one-liners, or mechanical workflows; the behavior
   should arise from 10x itself.
