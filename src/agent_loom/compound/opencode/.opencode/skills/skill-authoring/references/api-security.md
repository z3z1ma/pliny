...
When the agent executes this, the full command with expanded `$API_KEY` may appear in the conversation.
</the_problem>

<the_solution>
Use a wrapper script (e.g., `scripts/secure-api.sh`) that loads credentials internally.
...
3. **Add credential placeholders to `.env`** using profile naming (and gitignore it):
...
<credential_storage>
**Location:** `.env` (project-local; keep it out of version control)
...
4. **Keep credentials in a single local secrets file** (e.g., `.env`) - one central location, works everywhere
