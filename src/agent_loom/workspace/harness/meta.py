...
def harness_set_rm(*, name: str, root: Optional[Path] = None) -> Dict[str, Any]:
    ws_root = root.resolve() if root is not None else harness_root()
    ws = load_workspace(ws_root)
...
def harness_set_show(*, name: str, root: Optional[Path] = None) -> Dict[str, Any]:
    ws_root = root.resolve() if root is not None else harness_root()
    ws = load_workspace(ws_root)
    sets = ws.get("repo_sets", {})
    if not isinstance(sets, dict) or name not in sets:
...
def harness_set_ls(*, root: Optional[Path] = None) -> Dict[str, Any]:
    ws_root = root.resolve() if root is not None else harness_root()
    ws = load_workspace(ws_root)
    sets = ws.get("repo_sets", {})
