...
from agent_loom.workspace.constants import BULLET_RE, COMPONENT_HEADER
from agent_loom.workspace.state import iter_repos, ws_components_dir, ws_repos_dir


def component_md_path(root: Path, ws: dict, name: str) -> Path:
    return root / ws_components_dir(ws) / f"{name}.md"


def components_index_path(root: Path, ws: dict) -> Path:
    return root / ws_components_dir(ws) / "index.json"
...
def ensure_component_files(root: Path, ws: dict, repo_names: List[str]) -> None:
    for name in repo_names:
        p = component_md_path(root, ws, name)
        if not p.exists():
            atomic_write_text(p, COMPONENT_HEADER.format(name=name))


def refresh_components_index(root: Path, ws: dict) -> dict:
    """
    Build components/index.json from components/*.md (Depends on section) and workspace inventory.
    ...
    ensure_component_files(root, ws, names)

    forward: Dict[str, List[str]] = {}
    external: Dict[str, List[str]] = {}
    for name in names:
        md = component_md_path(root, ws, name).read_text(encoding="utf-8")
    ...
    atomic_write_json(components_index_path(root, ws), index)
    return index
